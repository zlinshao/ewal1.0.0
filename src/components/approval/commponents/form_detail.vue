        ...
        <el-form label-width='120px' v-for='(form,slither,index) in defineReport' :key='slither' v-if='visible && formData'>
          <VillageContainer :village="titleTips[index]">
            ...
            <!-- <el-row :gutter='10' v-for='(cell,cellIndex) in form' :key='cell.keyName' v-if='cell.type == "change"'>
              <el-col :span='cell.formSpan || formSpan' v-for='(child,child_index) in cell.children'
                :key='child_index'>
            ...
            <!-- 正常表单 -->
            <el-row :gutter='10' v-if='form.status != "change"'>
                ...
                <el-form-item :label='cell.label'>
                  ...
                  <!-- <template v-if='cell.type== "checkbox"'>
                    <div v-if='cell.children'>
                     ...
                     <el-select v-model="formData[cell.keyName]" :placeholder="cell.placeholder" v-else>
    ...
    resetForm () {
      ...
      for (let label in this.defineReport) {
        ...
        form.forEach(item => {
          ...
          if (item.children) {
            let newval = item.keyValue,
                isArr = (Object.prototype.toString.call(newval) === '[object Array]'),
                children = isArr ? item.children : item.children[0];
            children.forEach(child => {
              if(isArr){
                newval.push(child.keyValue)
              }else{
                newval[child.keyName] = child.keyValue
              }
            })
            formData[item.keyName] = newval
          }
        })
      }
