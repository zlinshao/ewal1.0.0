        ...
        this.$http.get(globalConfig.temporary_server + 'customer_renter', this.params).then(res => {
          if (res.code === 200) {
            ...
            for (let item of this.lordLists) {
              if (item.pay[0] == 1) {this.payTxt = "月付"}
              if (item.pay[0] == 2) {this.payTxt = "双月付"}
              if (item.pay[0] == 3) {this.payTxt = "季付"}
              if (item.pay[0] == 4) {this.payTxt = "半年付"}
              if (item.pay[0] == 5) {this.payTxt = "年付"}
            }
          } else {
            this.lordLists = [];
            this.lordCount = 0;
